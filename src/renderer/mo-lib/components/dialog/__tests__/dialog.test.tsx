...
describe('Test Modal Component', () => {
  test('Modal component default value', () => {
    ...
    expect(OK_FN).toHaveBeenCalled();

    fireEvent.click(buttonCancel);
    expect(CANCEL_FN).toHaveBeenCalled();
  });

  test('Modal component custom footer', () => {
    const TEST_FOOTER = <a data-testid={TEST_ID}>{TEST_ID}</a>;
    const wrapper = render(<Modal footer={TEST_FOOTER} visible />);
  ...
  test('Modal component centered', () => {
    const TEST_FOOTER = <a data-testid={TEST_ID}>{TEST_ID}</a>;
    const wrapper = render(<Modal footer={TEST_FOOTER} centered visible />);
