...
import { select, selectAll } from '@/common/dom';
import {
  sashHorizontalClassName,
  splitClassName,
  paneItemClassName,
} from '@/components/split/base';
...
function expectElementInOrNot(
  ...
  if (InDocument) {
    expect(ele?.parentElement?.style[horizontal ? 'height' : 'width']).not.toBe(
      '0px'
    );
  } else {
...
describe('Test Workbench Component', () => {
  ...
  test('Should Match Snapshots', () => {
    ...
    expect(
      render(
        <WorkbenchView {...workbenchModel()} auxiliaryBar={{ hidden: true }} />
      ).asFragment()
    ).toMatchSnapshot();
